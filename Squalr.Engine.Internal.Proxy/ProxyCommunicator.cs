namespace Squalr.Engine.Proxy
{
    ...
    public class ProxyCommunicator
    {
        ...
        /// </summary>
        private const String Proxy32Executable = "Squalr.Engine.Internal.Proxy32.exe";

        /// <summary>
        /// The 64 bit proxy service executable
        /// </summary>
        private const String Proxy64Executable = "Squalr.Engine.Internal.Proxy64.exe";
        ...
        private ProxyCommunicator()
        {
            ...
            try
            {
                ...
                Output.Log(LogLevel.Info, "Initialized 32 bit proxy service over pipe: " + pipeName32);
            }
            catch (Exception ex)
            {
                Output.Log(LogLevel.Fatal, "Error initializing 32 bit proxy service over pipe: " + pipeName32, ex);
            }

            try
            {
                this.Proxy64 = PipeDream.PipeDream.ClientInitialize<IProxyAssembler>(pipeName64);
                Output.Log(LogLevel.Info, "Initialized 64 bit proxy service over pipe: " + pipeName64);
            }
            catch (Exception ex)
            {
                Output.Log(LogLevel.Fatal, "Error initializing 64 bit proxy service over pipe: " + pipeName64, ex);
            }
        }
        ...
        private void StartServer(string executableName, string pipeName)
        {
            ...
            {
                Output.Log(LogLevel.Fatal, "Error starting proxy service, some features will not be available.", ex);
            }
        }
