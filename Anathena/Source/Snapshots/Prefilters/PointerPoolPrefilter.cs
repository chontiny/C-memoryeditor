...
namespace Ana.Source.Snapshots.Prefilter
{
    ...
    /// within some 'radius' of the pointer. Ie a pointer to 0x0080000 with a pointer size of 1kb will keep the chunk at that location.
    /// 
    ...
    /// </summary>
    class PointerPoolPrefilter : RepeatedTask, ISnapshotPrefilter, IProcessObserver
    {
        // Singleton instance of Prefilter
        private static Lazy<ISnapshotPrefilter> SnapshotPrefilterInstance = new Lazy<ISnapshotPrefilter>(() => { return new PointerPoolPrefilter(); }, LazyThreadSafetyMode.PublicationOnly);

        private EngineCore EngineCore;

        private const Int32 PointerSize = 512;
        private const Int32 RegionLimit = 64;
                ...
                Parallel.For(0, Math.Min(RegionList.Count, RegionLimit), Index =>
                {
                    ...
                    foreach (SnapshotElement Element in Region)
                    {
                        ...
                        if (Snapshot.ContainsAddress(Value))
                            FoundRegions.Add(new SnapshotRegion<Null>(Value, PointerSize));
