namespace Xilium.CefGlue
{
    ...
    public static unsafe class CefRuntime
    {
        ...
        public static void Load()
        {
            if (IntPtr.Size == 4)
            {
                libcef.SetDllDirectory(Path.Combine(Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location), "Libraries/32/"));
            }
            else if (IntPtr.Size == 8)
            {
                libcef.SetDllDirectory(Path.Combine(Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location), "Libraries/64/"));
            }
            Load(null);
        }
