...
(*
open Anathena.Source.Engine.OperatingSystems.Windows.Native;
open Anathena.Source.Utils.Extensions;
...
type Memory() =
    ...
    member ReadBytes(IntPtr ProcessHandle, IntPtr Address, Int32 Size, out Boolean Success) =
    ...
    member WriteBytes(IntPtr ProcessHandle, IntPtr Address, Byte[] ByteArray) =
        // Create the variable storing the number of bytes written
        Int32 BytesWritten;

        // Write the data to the target process
        if (NativeMethods.WriteProcessMemory(ProcessHandle, Address, ByteArray, ByteArray.Length, out BytesWritten)) then
            // Check whether the length of the data written is equal to the inital array
            if (BytesWritten == ByteArray.Length) then
                BytesWritten
        else 0
    ...
    member Allocate(IntPtr ProcessHandle, Int32 Size, MemoryProtectionFlags ?ProtectionFlags, MemoryAllocationFlags ?AllocationFlags) =
        // Initialize default arguments
        match ProtectionFlags with
            | None -> ProtectionFlags = MemoryProtectionFlags.ExecuteReadWrite
        match AllocationFlags with
            | None -> AllocationFlags = MemoryAllocationFlags.Commit

        // Allocate a memory page
        Address = NativeMethods.VirtualAllocEx(ProcessHandle, IntPtr.Zero, Size, AllocationFlags, ProtectionFlags)
    ...
    member OpenProcess(ProcessAccessFlags AccessFlags, Process Process) =
        NativeMethods.OpenProcess(AccessFlags, false, Process == null ? 0 : Process.Id)

    /// <summary>
    /// Closes an open object handle.
    /// </summary>
    /// <param name="Handle">A valid handle to an open object.</param>
    member CloseHandle(IntPtr Handle) =
        // Close the handle
    ...
    member Free(IntPtr ProcessHandle, IntPtr Address) =
        // Free the memory
    ...
    member MemoryProtectionFlags ChangeProtection(IntPtr ProcessHandle, IntPtr Address, int Size, MemoryProtectionFlags protection) =
    ...
    /// </summary>
    member IEnumerable<IntPtr> VirtualPages(IntPtr Handle, IntPtr StartAddress, IntPtr EndAddress,
    ...
    member IEnumerable<IntPtr> AllVirtualPages(IntPtr Handle)
    ...
    member IEnumerable<MemoryBasicInformation64> Query(IntPtr ProcessHandle, IntPtr StartAddress, IntPtr EndAddress, MemoryProtectionFlags RequiredProtection, MemoryProtectionFlags ExcludedProtection, MemoryTypeEnum AllowedTypes) =
