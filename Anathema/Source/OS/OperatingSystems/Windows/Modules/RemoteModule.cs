...
namespace Anathema.Source.OS.OperatingSystems.Windows.Modules
{
    /// <summary>
    ...
    public class RemoteModule : RemoteVirtualPage
    {
        ...
        {
            get { return WindowsOSInterface.Native.MainModule.BaseAddress == BaseAddress; }
        }
        ...
        public override bool IsValid
        {
            get
            {
                return base.IsValid && WindowsOSInterface.Native.Modules.Cast<ProcessModule>().Any(m => m.BaseAddress == BaseAddress && m.ModuleName == Name);
            }
        }
        ...
        {
            // Create the tuple
            Tuple<String, SafeMemoryHandle> Tuple = System.Tuple.Create(FunctionName, WindowsOSInterface.Handle);
            ...
            try
            {
                ...
                RemoteFunction Function = new RemoteFunction(WindowsOSInterface, new IntPtr(Native.BaseAddress.ToInt64() + Offset), FunctionName);
