...
namespace Anathema.Source.OS.OperatingSystems.Windows.Memory
{
    ...
    public class RemoteAllocation : RemoteVirtualPage, IDisposableState
    {
        ...
        internal RemoteAllocation(WindowsOSInterface WindowsOSInterface, Int32 Size, MemoryProtectionFlags Protection = MemoryProtectionFlags.ExecuteReadWrite,
                                 Boolean MustBeDisposed = true)
            : base(WindowsOSInterface, MemoryCore.Allocate(WindowsOSInterface.Handle, Size, Protection))
        {
            // Set local vars
        ...
        public virtual void Dispose()
        {
            if (!IsDisposed)
            {
                ...
                WindowsOSInterface.Memory.Deallocate(this);
