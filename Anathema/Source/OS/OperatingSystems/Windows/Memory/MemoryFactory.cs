...
namespace Anathema.Source.OS.OperatingSystems.Windows.Memory
{
    ...
    public class MemoryFactory : IFactory
    {
        ...
        /// </summary>
        protected readonly WindowsOSInterface WindowsOSInterface;
        /// <summary>
        ...
        {
            return MemoryCore.Query(WindowsOSInterface.Handle, StartAddress, EndAddress, RequiredProtection, ExcludedProtection, AllowedTypes).Select(x => new RemoteVirtualPage(WindowsOSInterface, x.BaseAddress));
        }

        public IEnumerable<RemoteVirtualPage> AllVirtualPages()
        {
            return MemoryCore.Query(WindowsOSInterface.Handle, IntPtr.Zero, IntPtr.Zero.MaxValue(), 0, 0,
                MemoryTypeEnum.None | MemoryTypeEnum.Private | MemoryTypeEnum.Image | MemoryTypeEnum.Mapped).Select(x => new RemoteVirtualPage(WindowsOSInterface, x.BaseAddress));
        }
        ...
        /// </summary>
        /// <param name="WindowsOSInterface">The reference of the <see cref="OS.OperatingSystems.WindowsOSInterface"/> object.</param>
        internal MemoryFactory(WindowsOSInterface WindowsOSInterface)
        {
            // Save the parameter
            this.WindowsOSInterface = WindowsOSInterface;
        ...
        public RemoteAllocation Allocate(Int32 Size, MemoryProtectionFlags Protection = MemoryProtectionFlags.ExecuteReadWrite, Boolean MustBeDisposed = true)
        {
            ...
            RemoteAllocation Memory = new RemoteAllocation(WindowsOSInterface, Size, Protection, MustBeDisposed);
