namespace Ana.Source.Snapshots.Deprecating
{
    ...
    /// </summary>
    internal abstract class SnapshotDeprecating : IEnumerable
    {
        /// <summary>
        ...
        public Int32 Alignment
        {
            ...
            set
            {
                ...
                foreach (SnapshotRegionDeprecating region in this)
                {
        ...
        /// </summary>
        protected IEnumerable<SnapshotRegionDeprecating> SnapshotRegions { get; set; }

        /// <summary>
        /// Gets or sets the collection of deallocated regions found when reading memory
        /// </summary>
        protected List<SnapshotRegionDeprecating> DeallocatedRegions { get; set; }
        ...
        public SnapshotElementDeprecating this[Int32 index]
        {
            get
            {
                foreach (SnapshotRegionDeprecating region in this)
        ...
        public abstract SnapshotDeprecating Clone();
        ...
        public abstract SnapshotDeprecating<NewLabelType> CloneAs<NewLabelType>() where NewLabelType : struct;
        ...
        public IEnumerable<SnapshotRegionDeprecating> GetSnapshotRegions()
        {
    ...
    internal class SnapshotDeprecating<LabelType> : SnapshotDeprecating where LabelType : struct
    {
        /// <summary>
        /// Initializes a new instance of the <see cref="SnapshotDeprecating{LabelType}"/> class
        /// </summary>
        public SnapshotDeprecating()
        {
            this.SnapshotRegions = null;
            this.DeallocatedRegions = new List<SnapshotRegionDeprecating>();
        ...
        public SnapshotDeprecating(IEnumerable<SnapshotRegionDeprecating> snapshotRegions)
        {
            this.SnapshotRegions = snapshotRegions == null ? null : snapshotRegions.Select((Func<SnapshotRegionDeprecating, SnapshotRegionDeprecating<LabelType>>)(x => (SnapshotRegionDeprecating<LabelType>)new SnapshotRegionDeprecating<LabelType>(x)));
            this.DeallocatedRegions = new List<SnapshotRegionDeprecating>();
            this.DeallocatedRegionLock = new Object();
            this.MergeRegions();
        }
        ...
        /// </summary>
        /// <returns>A shallow clone of this snapshot</returns>
        public override SnapshotDeprecating Clone()
        {
            List<SnapshotRegionDeprecating<LabelType>> regions = new List<SnapshotRegionDeprecating<LabelType>>();

            if (this.GetRegionCount() > 0)
            {
                foreach (SnapshotRegionDeprecating region in this.GetSnapshotRegions())
                {
                    regions.Add(new SnapshotRegionDeprecating<LabelType>(region));
                    regions.Last().SetCurrentValues(region.GetCurrentValues());
                    regions.Last().SetPreviousValues(region.GetPreviousValues());
                }
            }

            SnapshotDeprecating<LabelType> clonedSnapshot = new SnapshotDeprecating<LabelType>(regions);
        ...
        /// <typeparam name="NewLabelType">The label type of the snapshot clone</typeparam>
        /// <returns>A shallow clone of this snapshot with the new label type</returns>
        public override SnapshotDeprecating<NewLabelType> CloneAs<NewLabelType>()
        {
            return new SnapshotDeprecating<NewLabelType>(this.Clone().GetSnapshotRegions());
        }
        ...
        {
            foreach (SnapshotRegionDeprecating snapshotRegion in this)
            {
        ...
        public override void ReadAllSnapshotMemory()
        {
            ...
            IEnumerable<SnapshotRegionDeprecating> newRegions = this.MaskDeallocatedRegions();
            ...
            foreach (SnapshotRegionDeprecating snapshotRegion in newRegions)
            {
                Boolean readSuccess;
        ...
        {
            List<SnapshotRegionDeprecating<LabelType>> candidateRegions = new List<SnapshotRegionDeprecating<LabelType>>();
            ...
            foreach (SnapshotRegionDeprecating<LabelType> snapshotRegion in this)
            {
                candidateRegions.AddRange(snapshotRegion.GetValidRegions());
            }

            // Mask the regions against the original snapshot (this snapshot)
            IEnumerable<SnapshotRegionDeprecating<LabelType>> validRegions = this.MaskRegions(this, candidateRegions);

            // Shrink the regions based on their element type
            if (validRegions != null && validRegions.Count() > 0)
            {
                foreach (SnapshotRegionDeprecating<LabelType> region in validRegions)
                {
                    region.RelaxRegion();
                }
            }
        ...
        public void AddSnapshotRegions(IEnumerable<SnapshotRegionDeprecating<LabelType>> snapshotRegions)
        {
            List<SnapshotRegionDeprecating<LabelType>> newRegions = this.SnapshotRegions == null ? new List<SnapshotRegionDeprecating<LabelType>>() : ((IEnumerable<SnapshotRegionDeprecating<LabelType>>)this.SnapshotRegions).ToList();
            newRegions.AddRange(snapshotRegions);
            this.SnapshotRegions = newRegions.ToArray();
            Array.Sort((SnapshotRegionDeprecating<LabelType>[])this.SnapshotRegions, (x, y) => x.BaseAddress.ToUInt64().CompareTo(y.BaseAddress.ToUInt64()));
        ...
        {
            foreach (SnapshotRegionDeprecating<LabelType> region in this)
            {
        ...
        private IEnumerable<SnapshotRegionDeprecating<LabelType>> MaskDeallocatedRegions()
        {
            ...
            List<SnapshotRegionDeprecating<LabelType>> newSnapshotRegions = this.SnapshotRegions.Select(x => (SnapshotRegionDeprecating<LabelType>)x).ToList();

            // Remove invalid items from collection
            foreach (SnapshotRegionDeprecating<LabelType> region in this.DeallocatedRegions)
            {
                newSnapshotRegions.Remove(region);
            }

            // Get current memory regions
            SnapshotDeprecating<LabelType> mask = null;// SnapshotManager.GetInstance().CollectSnapshot(useSettings: false, usePrefilter: false).CloneAs<LabelType>();

            // Mask each region against the current virtual memory regions
            IEnumerable<SnapshotRegionDeprecating<LabelType>> maskedRegions = this.MaskRegions(mask, this.DeallocatedRegions);
