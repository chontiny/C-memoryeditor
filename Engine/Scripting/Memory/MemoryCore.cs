namespace Squalr.Engine.Scripting.Memory
{
    ...
    internal class MemoryCore : IMemoryCore
    {
        ...
        public Byte[] GetAssemblyBytes(String assembly, UInt64 address)
        {
            ...
            String message;
            String innerMessage;
            Byte[] result = Eng.GetInstance().Architecture.GetAssembler().Assemble(ProcessAdapterFactory.GetProcessAdapter().IsOpenedProcess32Bit(), assembly, address.ToIntPtr(), out message, out innerMessage);
        ...
        public Byte[] CollectOriginalBytes(UInt64 address, Int32 injectedCodeSize)
        {
            ...
            IEnumerable<NormalizedInstruction> instructions = Eng.GetInstance().Architecture.GetDisassembler().Disassemble(originalBytes, ProcessAdapterFactory.GetProcessAdapter().IsOpenedProcess32Bit(), address.ToIntPtr());
